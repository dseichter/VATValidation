...
def start_workflow(payload):
    ...
    # Use hmrc for GB VAT numbers, bzst if selected, swiss for CHE, otherwise use vies
    if payload["foreignvat"].upper().startswith("GB"):
        response = validate_hmrc.start_validation(payload)
    elif payload["foreignvat"].upper().startswith("CHE"):
        response = validate_swiss.start_validation(payload)
